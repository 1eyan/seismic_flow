...
DATASET_NEIGHBORS="${DATASET_NEIGHBORS:-/data/shared/测试数据/h5/anchor_patch_e2ev1/infer_query_context.npz}"
...
PATCH_METRIC_WEIGHTS="${PATCH_METRIC_WEIGHTS:-1.0,1.0,0.5,0.5}"
TRACE_SORT_KEYS="${TRACE_SORT_KEYS:-rx,ry,sx,sy}"
EPOCH_REPEAT="${EPOCH_REPEAT:-1}"
TARGET_MODE="${TARGET_MODE:-supervised}"
COORD_AUG_SCALE="${COORD_AUG_SCALE:-0.0}"
ALLOW_COORD_STATS_FALLBACK="${ALLOW_COORD_STATS_FALLBACK:-true}"
REGULAR_HOLDOUT_NPZ="${REGULAR_HOLDOUT_NPZ:-}"
REGULAR_TASK_PROB="${REGULAR_TASK_PROB:-0.3}"
...
accelerate launch --config_file "${ACCELERATE_CONFIG}" --main_process_port 29502 train_fpmV3_ddp.py \
