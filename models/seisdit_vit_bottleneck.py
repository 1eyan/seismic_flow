...
class DiTBlockVitBottleneck(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, t0_ms=0.0, dt_ms=1.0, downsample_stride=1):
        super().__init__()
...
class SeisDiTRope(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, condL=None, log_tau=None, time_axis=None, training=False):
        ...
        if condL is not None:
            rx, ry, sx, sy = condL
            '''rx_mean = rx.mean(dim=-1, keepdim=True)
            ry_mean = ry.mean(dim=-1, keepdim=True)
            sx = sx - rx_mean
            sy = sy - ry_mean
            rx = rx - rx_mean
            ry = ry - ry_mean'''
